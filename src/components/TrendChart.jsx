...
export function TrendChart({ spending }) {
  const data = useMemo(() => {
    ...
    const byMonth = months.map((m) => {
      const rows = spending.filter((r) => r.date && r.date.startsWith(m));
      const parts = {};
      let total = 0;
      mains.forEach((k) => {
    ...
    const max = Math.max(1, ...byMonth.map((x) => x.total));
    return { months, byMonth, max, mains };
  }, [spending]);

  const W = 700, H = 190, padL = 8, padB = 26, barGap = 8;
  const barW = (W - padL * 2) / 12 - barGap;

  return (
    <div style={{ marginBottom: 26 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 6 }}>
        <div style={{ fontSize: 13, color: "#6b5f54" }}>近 12 個月支出趨勢</div>
      ...
      <div style={{ overflowX: "auto" }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", minWidth: 480, marginTop: 8 }}>
          {data.byMonth.map((x, i) => {
            const xPos = padL + i * ((W - padL * 2) / 12) + barGap / 2;
            let y = H - padB;
            ...
            return (
              <g key={x.m}>
                {data.mains.map((k) => {
                  ...
                  return (
                    <rect key={k} x={xPos} y={y} width={barW} height={h} fill={MAIN_COLORS[k]} rx={2}>
                      <title>{x.m} {k}：{fmt(x.parts[k])}</title>
                    </rect>
                  );
                })}
                {x.total > 0 && (
                ...
                <text x={xPos + barW / 2} y={H - padB + 15} textAnchor="middle" fontSize="10" fill="#9a8d80">{x.m.slice(5)}月</text>
              </g>
            );
          })}
