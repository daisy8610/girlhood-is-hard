...
function BestTag() {
  return (
    <span style={{ fontSize: 10.5, padding: "1px 7px", borderRadius: 9999, background: "#000", color: "#fff", marginLeft: 6, whiteSpace: "nowrap" }}>最划算</span>
  );
}
...
function Pill({ active, onClick, children }) {
  return (
    <button
      onClick={onClick}
      style={{
        fontSize: 12, padding: "6px 14px", borderRadius: 9999, border: "1px solid " + (active ? "#000" : "#EBE8E4"),
    ...
    <div className="row-hover" style={{
      ...
      <div style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
        <div style={{ textAlign: "right" }}>
          <div className="mono" style={{ fontSize: 13.5, whiteSpace: "nowrap" }}>{fmt(r.price)}</div>
        ...
        <div>
          {Object.entries(grouped).map(([c, prods]) => (
            <div key={c} style={{ marginBottom: 18 }}>
              <div className="serif" style={{ fontSize: 15, fontWeight: 500, color: "#000", borderBottom: "1px solid #EBE8E4", paddingBottom: 6, marginBottom: 8 }}>{c}</div>
          ...
          {byClinic.map(([clinic, list, latest]) => (
            <div key={clinic} style={{ border: "1px solid #EBE8E4", borderRadius: 12, padding: "12px 14px", marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, marginBottom: 4 }}>
                <div className="serif" style={{ fontSize: 15, fontWeight: 500, color: "#000" }}>{clinic}</div>
                <div style={{ fontSize: 11, color: "#A59F97", whiteSpace: "nowrap" }}>
