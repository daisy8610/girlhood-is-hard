  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <div key={r.id} className="row-hover" style={{
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 14 }}>
                  ...
                  {r.item} <span style={{ color: "#A59F97", fontSize: 12 }}>{r.sub ? `· ${r.sub}` : ""}</span>
                </div>
                <div style={{ fontSize: 11, color: "#A59F97", overflowWrap: "anywhere" }}>{r.date || "—"} · {r.place || "—"}{r.note ? ` · ${r.note}` : ""}</div>
              </div>
        ...
        {filtered.length === 0 && <div style={{ color: "#A59F97", fontSize: 13, padding: 12 }}>找不到符合的紀錄</div>}
      </div>
    </div>
  );
}
