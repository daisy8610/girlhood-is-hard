...
function SettingsCard({ title, sub, children }) {
  return (
    <div style={{ background: "#fff", border: "1px solid var(--ed-stone)", borderRadius: "var(--r-lg)", padding: 18, marginBottom: 14 }}>
      <div style={{ fontSize: "var(--fs-lg)", fontWeight: 500, color: "var(--ed-ink)" }}>{title}</div>
      {sub && <div style={{ fontSize: "var(--fs-sm)", color: "var(--ed-smoke)", marginTop: 3, marginBottom: 12, fontWeight: 400 }}>{sub}</div>}
      {children}
    </div>
  );
}
...
export function SettingsPage({
  ...
  const btn = { border: "1px solid var(--ed-stone)", color: "var(--ed-ink)", background: "#fff", borderRadius: 9999, padding: "9px 18px", fontSize: "var(--fs-md)" };
  const btnPrimary = { ...btn, border: "1px solid var(--ed-ink)", background: "var(--ed-ink)", color: "#fff", fontWeight: 500 };
  ...
  return (
    <div>
      ...
      <SettingsCard title="Google 日曆" sub="連結後，新增消費紀錄時勾選「同步到 Google 日曆」，就會在你的日曆建一筆行程">
        {googleLinked ? (
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <span style={{ fontSize: "var(--fs-md)", color: "var(--ed-ink)" }}>✓ 已連結</span>
      ...
      <SettingsCard title="年度預算上限" sub="總覽的進度條會以這個數字計算">
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ fontSize: "var(--fs-lg)", color: "var(--ed-smoke)" }}>NT$</span>
          <input
            type="number" value={capInput} onChange={(e) => setCapInput(e.target.value)}
            style={{ flex: 1, padding: "9px 12px", borderRadius: 9999, border: "1px solid var(--ed-stone)", fontSize: "var(--fs-xl)", minWidth: 0 }} className="mono"
          />
      ...
      <SettingsCard title="備份與還原" sub="資料存在你的 Supabase 雲端資料庫；仍建議偶爾下載一份備份">
        ...
        {importErr && (
          <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: "var(--r-md)", background: "var(--ed-surface)", color: "var(--ed-ink)", fontSize: "var(--fs-md)" }}>
            ⚠️ 無法匯入：{importErr}
          </div>
        )}

        {pending && (
          <div style={{ marginTop: 12, border: "1px solid var(--ed-stone)", borderRadius: "var(--r-lg)", padding: 14, background: "var(--ed-surface)" }}>
            <div style={{ fontSize: "var(--fs-lg)", fontWeight: 500, color: "var(--ed-ink)", marginBottom: 8 }}>
              還原預覽{pending.exportedAt ? `（備份於 ${String(pending.exportedAt).slice(0, 10)}）` : "（備份日期不明）"}
            </div>
            <div style={{ overflowX: "auto" }}>
              <table style={{ borderCollapse: "collapse", fontSize: "var(--fs-sm)", width: "100%" }}>
                <thead>
                  <tr>
                    <th style={{ textAlign: "left", padding: "4px 8px", borderBottom: "1px solid var(--ed-stone)" }}></th>
                    {BACKUP_KEYS.map((k) => (
                      <th key={k} style={{ textAlign: "right", padding: "4px 8px", borderBottom: "1px solid var(--ed-stone)", whiteSpace: "nowrap" }}>{BACKUP_LABELS[k]}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td style={{ padding: "4px 8px", color: "var(--ed-ash)", whiteSpace: "nowrap" }}>目前</td>
                    {BACKUP_KEYS.map((k) => (
                      <td key={k} className="mono" style={{ textAlign: "right", padding: "4px 8px" }}>{counts[k]}</td>
                    ))}
                  </tr>
                  <tr>
                    <td style={{ padding: "4px 8px", color: "var(--ed-ash)", whiteSpace: "nowrap" }}>備份檔</td>
                    {BACKUP_KEYS.map((k) => (
                      <td key={k} className="mono" style={{ textAlign: "right", padding: "4px 8px", fontWeight: 600, color: pending.payload[k].length !== counts[k] ? "var(--ed-ink)" : "inherit" }}>
            ...
            <div style={{ fontSize: "var(--fs-sm)", color: "var(--ed-smoke)", margin: "10px 0" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button onClick={() => { applyImport(pending.payload); setPending(null); }} style={btnPrimary}>
                確認還原
              </button>
      ...
      <SettingsCard title="清空所有資料" sub="清空後雲端會是空的；要重新放資料請用上方匯入或執行 SEED_DATA.sql（清空前會自動下載備份）">
        {clearConfirm ? (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ fontSize: "var(--fs-md)", color: "var(--ed-ink)", fontWeight: 500 }}>確定要清空嗎？</span>
            <button onClick={() => { clearAllData(); setClearConfirm(false); }} style={btnPrimary}>
              確定清空
            </button>
      ...
      <SettingsCard title="關於">
        <div style={{ fontSize: "var(--fs-md)", color: "var(--ed-smoke)", lineHeight: 1.8, fontWeight: 400 }}>
      ...
      <div style={{ textAlign: "center", fontSize: "var(--fs-sm)", color: "var(--ed-ash)", marginTop: 8, lineHeight: 1.7 }}>
