  ...
  return (
    <div style={{ marginBottom: 26 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <div style={{ fontSize: 14, color: "#777169" }}>儲值金與剩餘堂數</div>
        {!adding && (
          ...
          editingId === r.id ? (
            ...
            <div key={r.id} style={{ border: "1px solid #EBE8E4", borderRadius: 14, padding: "13px 15px", background: "#fff" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 4 }}>
                <div style={{ fontSize: 14, color: "#777169" }}>{r.name}</div>
                <RowActions onEdit={() => setEditingId(r.id)} onDelete={() => h.del(r.id)} />
              </div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
                <div className="mono" style={{ fontSize: 22, fontWeight: 500, marginTop: 4, color: "#000" }}>
                  {r.value == null ? "？" : r.unit === "元" ? fmt(r.value) : r.value}
                  {r.unit !== "元" && r.value != null && <span style={{ fontSize: 14, marginLeft: 3, color: "#777169" }}>{r.unit}</span>}
                </div>
                {r.value != null && r.value > 0 && r.unit !== "元" && (
                  <button
                    ...
                    style={{ border: "1px solid #EBE8E4", borderRadius: 9999, padding: "2px 10px", fontSize: 13 }}
                  >
                    − 用一次
                  </button>
                )}
              </div>
              <div style={{ fontSize: 12, color: "#7D776F", marginTop: 4 }}>{r.updated || "—"}{r.note ? ` · ${r.note}` : ""}</div>
            </div>
          )
        )}
      </div>
